...
#import matchtable
cl <- read.table('Z:/Bohan/_Stripe Rust/Genome/pangenome/matchtable/0915_Yr/matchtable_0_1.txt', header = T , stringsAsFactors = F)
...
write.csv(output_data, "Z:/Bohan/_Stripe Rust/Genome/pangenome/matchtable/0915_Yr/30_pan_gene-count_out.csv", row.names = FALSE)
...
#Generate stats 
data <- read.table("Z:/Bohan/_Stripe Rust/Genome/pangenome/matchtable/0915_Yr/30_pan_gene-count_out.csv", header=TRUE, sep=",")
...
write.csv(graph.data, "Z:/Bohan/_Stripe Rust/Genome/pangenome/matchtable/0915_Yr/30_gene-numbers_graph-data.csv", row.names = FALSE)
...
ggplot(output_data, aes(x=genomes, y=total_genes, font=2, group=genomes)) +
  geom_boxplot()+      
  #geom_smooth(inherit.aes = F, aes(x=genomes, y=total_genes), se=F)+
  geom_line()+
  theme_gray()+
...
ggplot(output_data, aes(x=genomes, y=total_core, font=2, group=genomes)) +
  geom_boxplot()+      
  #geom_smooth(inherit.aes = F, aes(x=genomes, y=total_genes), se=F)+
  geom_line()+
  theme_gray()+
